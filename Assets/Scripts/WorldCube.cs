...
public class WorldCube : MonoBehaviour
{
    ...
    [SerializeField] float triggerThreshold = 0.5f;  // Distance from center needed to trigger (HIGHER = need to be CLOSER to edge)
    [SerializeField] float maxDetectorDistance = 3.0f; // Max distance from detector position allowed
    ...
    // Cooldown to prevent rapid rotations
    private float lastRotationTime = -999f;
    private float rotationCooldown = 1.0f; // Minimum time between rotations
    ...
    public static WorldCube Instance { get; private set; }
    ...
    /// <summary>
    /// Set up the face turn detectors - now just for position reference, no triggers.
    /// </summary>
    private void SetupFaceTurnDetectors()
    {
        // Face turn detectors are now just position references
        // No trigger setup needed - we'll use position-based detection
        Debug.Log("WorldCube: Face turn detectors set up as position references only");
    }
    
    /// <summary>
    /// Check if the player should trigger a cube rotation based on position and input.
    /// Returns the direction to rotate if conditions are met, otherwise null.
    /// </summary>
    public RotationDirection? CheckForRotationTrigger(Vector3 playerPosition, Vector2 inputDirection)
    {
        ...
        // Check if input is significant enough
        if (inputDirection.magnitude < 0.3f)
        {
            return null;
        }

        // Debug input direction for troubleshooting
        if (inputDirection.magnitude > 0.3f)
        {
            Debug.Log($"WorldCube: Input detected - Direction: {inputDirection}, Magnitude: {inputDirection.magnitude:F2}");
        }

        // Check each direction with higher thresholds to prevent accidental triggers
        if (inputDirection.y > 0.5f && ShouldTriggerDirection(playerPosition, RotationDirection.Up, faceTurnDetectorNorth))
        {
            Debug.Log($"WorldCube: Triggering UP rotation - Input: {inputDirection}");
            return RotationDirection.Up;
        }
        
        if (inputDirection.y < -0.5f && ShouldTriggerDirection(playerPosition, RotationDirection.Down, faceTurnDetectorSouth))
        {
            Debug.Log($"WorldCube: Triggering DOWN rotation - Input: {inputDirection}");
            return RotationDirection.Down;
        }

        if (inputDirection.x > 0.5f && ShouldTriggerDirection(playerPosition, RotationDirection.Right, faceTurnDetectorEast))
        {
            Debug.Log($"WorldCube: Triggering RIGHT rotation - Input: {inputDirection}");
            return RotationDirection.Right;
        }
        
        if (inputDirection.x < -0.5f && ShouldTriggerDirection(playerPosition, RotationDirection.Left, faceTurnDetectorWest))
        {
            Debug.Log($"WorldCube: Triggering LEFT rotation - Input: {inputDirection}");
    ...
    /// <summary>
    /// Check if the player is past the trigger position for a specific direction.
    /// Player must be: 1) Close enough to cube edge, AND 2) Close to the detector position.
    /// </summary>
    private bool ShouldTriggerDirection(Vector3 playerPosition, RotationDirection direction, GameObject detector)
    {
        if (detector == null || cubeRoot == null) return false;
        
        Vector3 cubeCenter = cubeRoot.transform.position;
        Vector3 detectorPosition = detector.transform.position;
        
        // First check: Player must be close enough to cube edge in that direction
        // Higher triggerThreshold = need to be closer to edge to trigger (more intuitive)
        bool closeEnoughToEdge = false;
        float distanceFromCenter = 0f;
        
        switch (direction)
        {
            case RotationDirection.Up:
                distanceFromCenter = playerPosition.z - cubeCenter.z;
                closeEnoughToEdge = distanceFromCenter >= triggerThreshold;
                break;
                
            case RotationDirection.Down:
                distanceFromCenter = cubeCenter.z - playerPosition.z;
                closeEnoughToEdge = distanceFromCenter >= triggerThreshold;
                break;
                
            case RotationDirection.Right:
                distanceFromCenter = playerPosition.x - cubeCenter.x;
                closeEnoughToEdge = distanceFromCenter >= triggerThreshold;
                break;
                
            case RotationDirection.Left:
                distanceFromCenter = cubeCenter.x - playerPosition.x;
                closeEnoughToEdge = distanceFromCenter >= triggerThreshold;
                break;
        }
        
        // Second check: Player must be close to the detector position
        float distanceToDetector = Vector3.Distance(playerPosition, detectorPosition);
        bool nearDetector = distanceToDetector <= maxDetectorDistance;
        
        // Both conditions must be true
        bool shouldTrigger = closeEnoughToEdge && nearDetector;
        
        // Enhanced debug log for all directions
        if (distanceFromCenter >= triggerThreshold * 0.5f) // Show debug more often
        {
            Debug.Log($"WorldCube: {direction} check - Player pos: {playerPosition}, Cube center: {cubeCenter}, Detector pos: {detectorPosition}");
            Debug.Log($"WorldCube: {direction} distances - Center: {distanceFromCenter:F2}>={triggerThreshold}, Detector: {distanceToDetector:F2}<={maxDetectorDistance}, Trigger: {shouldTrigger}");
        }
    ...
    {
        isRotating = true;
        lastRotationTime = Time.time; // Record rotation time for cooldown
